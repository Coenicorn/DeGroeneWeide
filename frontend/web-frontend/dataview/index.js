...
function addLogLine(parent, title, content, color, data, func) {
    ...
    newElm.innerHTML = `<p class="log-text" style="background-color: ${color}"><span>${new Date().toLocaleTimeString().split(" ")[0]}</span><span>method:<b>${title}</b></span>${data.source?"<span>from:<b>"+data.source+"</b></span>":""}<span class="view-content code">${content}</span>${(data.isResponse && data.responseCode)?"<span>" + data.responseCode + "</span>":""}<span style="height: fit-content; color:${data.isResponse?'red':'green'}"><b>${data.isResponse?"[->":"[<-"}</b></span></p>`;
    newElm.onclick = func;
    func();
